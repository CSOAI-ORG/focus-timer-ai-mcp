#!/usr/bin/env python3
"""MEOK AI Labs — focus-timer-ai-mcp MCP Server. Run focus timers with distraction blocking recommendations."""
...
server = Server("focus-timer-ai-mcp")
...
async def handle_list_tools() -> list[Tool]:
    return [
        Tool(name="start_focus", description="Start a focus timer", inputSchema={"type":"object","properties":{"minutes":{"type":"number"}},"required":["minutes"]}),
    ]
    ...
    async with stdio_server(server._read_stream, server._write_stream) as (read_stream, write_stream):
        await server.run(
            ...
            InitializationOptions(
                server_name="focus-timer-ai-mcp",
